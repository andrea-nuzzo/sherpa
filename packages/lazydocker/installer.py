...
class LazydockerInstaller(BaseInstaller):
    """Installer for lazydocker - A simple terminal UI for docker and docker-compose"""

    def __init__(self, package_name):
        super().__init__(package_name)
        self.binary_name = "lazydocker"
    
    def install_software(self):
        """Install lazydocker using system package manager or GitHub releases"""
    ...
    def _install_via_package_manager(self):
        """Install lazydocker via system package manager"""
        ...
        logger.info(f"Installing lazydocker using: {package_manager}")
        
        result = self._run_command(full_command)
        
        if result.returncode == 0:
            logger.info("lazydocker installed successfully via package manager")
            return True
        else:
    ...
    def _install_from_github(self):
        ...
        if os_type == "macos":
            if arch == "arm64":
                filename = "lazydocker_*_Darwin_arm64.tar.gz"
            else:
                filename = "lazydocker_*_Darwin_x86_64.tar.gz"
        elif os_type in ["linux", "debian", "redhat", "arch"]:
            if arch == "arm64":
                filename = "lazydocker_*_Linux_arm64.tar.gz"
            else:
                filename = "lazydocker_*_Linux_x86_64.tar.gz"
        elif os_type == "windows":
            filename = "lazydocker_*_Windows_x86_64.zip"
        else:
        ...
        LATEST_URL=$(curl -s https://api.github.com/repos/jesseduffield/lazydocker/releases/latest | grep browser_download_url | grep '{filename.replace('*', '[0-9.]+')}' | head -1 | cut -d '"' -f 4)
        ...
        curl -L -o lazydocker.tar.gz "$LATEST_URL"
        ...
        if result.returncode == 0:
            logger.info("lazydocker installed successfully from GitHub")
            return True
        else:
    ...
    def uninstall_software(self):
        ...
        logger.info("Uninstalling lazydocker...")
    ...
    def _uninstall_via_package_manager(self):
        ...
        uninstall_commands = {
            "apt": "sudo apt remove -y lazydocker",
            "apt-get": "sudo apt-get remove -y lazydocker",
            "dnf": "sudo dnf remove -y lazydocker",
            "yum": "sudo yum remove -y lazydocker",
            "pacman": "sudo pacman -R --noconfirm lazydocker",
            "brew": "brew uninstall lazydocker",
            "winget": "winget uninstall jesseduffield.lazydocker",
            "choco": "choco uninstall -y lazydocker"
        }
        ...
        if result.returncode == 0:
            logger.info("lazydocker uninstalled successfully via package manager")
            return True
        else:
    ...
    def _uninstall_manual(self):
        """Manually remove lazydocker binary"""
        binary_path = self._find_lazydocker_binary()
        ...
        if binary_path:
            try:
                ...
                if result.returncode == 0:
                    logger.info("lazydocker binary removed manually")
                    return True
                else:
                    logger.error("Failed to remove lazydocker binary")
                    return False
            except Exception as e:
                logger.error(f"Manual removal failed: {e}")
                return False
        else:
            logger.warning("lazydocker binary not found")
            return True
    ...
    def _find_lazydocker_binary(self):
        """Find the lazydocker binary location"""
        import shutil
    ...
    def setup_integration(self):
        """Setup lazydocker integration - no additional setup needed"""
        logger.info("No additional integration setup needed for lazydocker")
        return True
    
    def uninstall_integration(self):
        """Remove lazydocker integration - no integration to remove"""
        logger.info("No integration to remove for lazydocker")
        return True